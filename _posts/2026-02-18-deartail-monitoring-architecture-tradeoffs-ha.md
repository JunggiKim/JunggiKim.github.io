...
메트릭은 APM 대시보드, 로그는 CloudWatch Logs, 트레이스는 별도 경로로 각각 확인해야 했다.
장애가 터지면 "어디서부터 봐야 하는지"부터 정리해야 했고, 도구 사이를 오가는 사이 분석 시간이 늘어났다.

소규모 팀에서 운영 전담 인력 없이 개발과 운영을 병행하는 상황이었기 때문에, 관측 경로가 분산되어 있으면 장애 대응 속도가 직접적으로 느려졌다.
...
| 로그 수집량 (일) | — |
| 트레이스 샘플링 비율 | — |
| 보관 기간 | 메트릭 —일, 로그 —일, 트레이스 —일 |

> 위 산정 조건은 운영 환경 기준이 확정되는 대로 업데이트할 예정이다.
...
3. 외부 API 장애를 내부 장애와 분리해 경보 품질이 개선됐다. 이전에는 외부 API 장애를 고객 문의가 들어온 뒤에야 인지하는 경우가 있었는데, synthetic check 도입 후 사전 알림으로 인지 시점이 앞당겨졌다.
